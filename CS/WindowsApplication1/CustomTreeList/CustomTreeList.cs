    ...
    internal class CustomTreeList : TreeList {
        ...
        private void CustomTreeList_DoubleClick(object sender, EventArgs e) {
            ...
            if (ht != null && ht.Node != null && ht.HitInfoType == HitInfoType.RowIndent) {
                ...
                if (ri != null && ri.IndentInfo.IndentItems.Count > 0) {
                    var levelByIndent = GetLevelByIndent(ri.IndentInfo, p, ViewInfo.RC.LevelWidth);
        ...
        private int GetLevelByIndent(IndentInfo indentInfo, Point point, int levelWidth) {
            var currentRect = indentInfo.Bounds;
            currentRect.Width = levelWidth;
            for (var i = 0; i < indentInfo.IndentItems.Count; i++) {
